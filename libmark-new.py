...
def detect_filetype(file):
    if isfile(file + '.csv') or isfile(file):
        import csv
        return 'csv'
    if isfile(file+'xls') or isfile(file+'xlsx'):
...
def doc_process(parameters):
    ...
    for idx, i in enumerate(individual_comment):
        ...
        if i == 6:
            bs_comment = comments_cells[0].add_paragraph(style='List Bullet').add_run(
                comment_statement[idx] + ' (Bonus Point)')
